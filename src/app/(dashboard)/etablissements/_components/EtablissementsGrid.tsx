...
// import { etablissementData } from './GroupePageSampleData';
import EtablissementCard from './EtablissementCard';
import { etablissementData } from './EtablissementPageSampleData';

const EtablissementsGrid = () => {
  const [view, setView] = useState<'grid' | 'list'>('grid');
  const [searchQuery, setSearchQuery] = useState('');
  const [minContacts, setMinContacts] = useState('');

  // Filtered Data
  const filteredGroups = etablissementData.filter(
    (group) =>
      ...
      <div className='h-[700px] overflow-auto'>
        <div className={`${view === 'grid' ? 'grid grid-cols-[repeat(auto-fit,minmax(350px,1fr))] gap-4' : 'flex flex-col gap-4' }`}>
          {filteredGroups.length > 0 ? (
            filteredGroups.map((group) => <EtablissementCard key={group.id} group={group} view={view} />)
          ) : (
...
export default EtablissementsGrid;
...
//       {/* Dynamic Layout */}
//       <div className={view === 'grid' ? 'grid grid-cols-[repeat(auto-fit,minmax(350px,1fr))] gap-4' : 'flex flex-col gap-4'}>
//         {etablissementData.map((group) => (
//           <EtablissementCard key={group.id} group={group} view={view} />
//         ))}
//       </div>
//     </div>
//   );
...
//         {etablissementData.map((group) => (
//           <EtablissementCard key={group.id} group={group} />
//         ))}
//       </div>
//     </div>
//   );
